...
class HandwrittenClassifier {
  ...
  async initialize(manifest: AppClassifierManifest, modelBuffer: ArrayBuffer, labels: string[]): Promise<void> {
    ...
    const inputMetadata = (
      this.session.inputMetadata as unknown as Record<string, { isTensor?: boolean; shape: Array<string | number> }>
    )[inputName];
  ...
  async recognize(imageData: ImageData): Promise<HandwrittenRecognitionResult> {
    ...
    for (const segment of segments) {
      const tensor = new ort.Tensor(
        ...
        this.layout === 'nchw'
          ? [1, this.channels, this.inputHeight, this.inputWidth]
          : [1, this.inputHeight, this.inputWidth, this.channels]
      );
...
function segmentIntoCharacters(
  imageData: ImageData
): Array<{ imageData: ImageData; boundingBox: { x: number; y: number; width: number; height: number } }> {
  ...
  return groups
    .map(group => cropCharacterRegion(imageData, group.left, group.top, group.right, group.bottom))
    .filter(
      (segment): segment is { imageData: ImageData; boundingBox: { x: number; y: number; width: number; height: number } } =>
        segment !== null
    );
}
...
function shouldMergeIntoCharacter(current: CharacterBox, next: CharacterBox): boolean {
  ...
  const smallMark =
    next.pixels <= current.pixels * 0.28 && Math.abs(centerX(next) - centerX(current)) <= Math.max(currentHeight, nextHeight) * 0.8;
...
function cropCharacterRegion(
  source: ImageData,
  left: number,
  top: number,
  right: number,
  bottom: number
): { imageData: ImageData; boundingBox: { x: number; y: number; width: number; height: number } } | null {
...
function buildSequenceSuggestions(
  perCharacterCandidates: CharacterCandidate[][],
  limit: number

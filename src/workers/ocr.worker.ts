...
function segmentIntoLines(imageData: ImageData): Array<{ imageData: ImageData; boundingBox: OcrLineResult['boundingBox'] }> {
  ...
  return mergedLines
    .map(line => cropLineRegion(imageData, line.left, line.top, line.right, line.bottom))
...
function shouldMergeIntoLine(
  ...
  const horizontalOverlap =
    Math.max(0, Math.min(line.right, component.right) - Math.max(line.left, component.left) + 1);
